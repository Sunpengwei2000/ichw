...
"""currency.py: A simple currency exchange routine using an online currency service.
...
def testC():
    ...
    """
    # Test case 1
    assert('{ "from" : "200 Uruguayan Pesos", "to" : "668.41898835558 Vanuatu Vatu",
           "success" : true, "error" : "" }' == currency_response('UYU', 'VUV', 200))
    # Test case 2
    assert('{ "from" : "", "to" : "", "success" : false,
           "error" : "Exchange currency code is invalid." }'            == currency_response('MOP', 'MTP', 16))
    # Test case 3
    assert('{ "from" : "", "to" : "", "success" : false, 
           "error" : "Source currency code is invalid." }'            == currency_response('AND', 'IQD', 50))      
